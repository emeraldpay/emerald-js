  ...
  var EthApi = function () {
    ...
    _createClass(EthApi, [{
      ...
      value: function estimateGas(call) {
        ...
        return this.rpc.call('eth_estimateGas', [txData]).then(function (gas) {
      ...
      value: function getTransactionCount(address) {
        ...
        return this.rpc.call('eth_getTransactionCount', [address, blockNumber]).then(_convert2.default.toNumber);
      }
    }, {
      key: 'getTransactionReceipt',
      value: function getTransactionReceipt(hash) {
        return this.rpc.call('eth_getTransactionReceipt', [hash]).then(_format2.default.transactionReceipt);
      }
    }, {
      ...
      value: function getTransaction(hash) {
        return this.rpc.call('eth_getTransactionByHash', [hash]).then(_format2.default.transaction);
      }
    }, {
      ...
      value: function getAddressTransactions(address, blockNumFloor, blockNumCeil, toOrFrom, standardOrContract, beginPageIndex, endPageIndex, orderByOldest) {
        return this.rpc.call('geth_getAddressTransactions', [address, blockNumFloor, blockNumCeil, toOrFrom, standardOrContract, beginPageIndex, endPageIndex, orderByOldest]);
      }
    }]);
    ...
    _createClass(ExtApi, [{
      ...
      value: function getBlocks(from, to) {
        ...
        for (var i = from; i <= to; i += 1) {
          requests.push(this.rpc.newBatchRequest('eth_getBlockByNumber', [_format2.default.toHex(i), false]));
        }
      ...
      value: function getBlocksByNumbers(number) {
        ...
        var requests = formattedNumber.map(function (blockNumber) {
          return _this.rpc.newBatchRequest('eth_getBlockByNumber', [blockNumber, false]);
        });
        return this.rpc.batch(requests).then(formatBatchBlockResponse);
      }
    }, {
      ...
      value: function getBalances(addresses) {
        ...
        var requests = addresses.map(function (a) {
          return _this2.rpc.newBatchRequest('eth_getBalance', [a, blockNumber], function (resp) {
      ...
      value: function getTransactions(hashes) {
        ...
        var requests = hashes.map(function (h) {
          return _this3.rpc.newBatchRequest('eth_getTransactionByHash', [h]);
        });
      ...
      value: function batchCall(calls) {
        ...
        var requests = calls.map(function (c) {
          return _this4.rpc.newBatchRequest('eth_call', [{ to: c.to, data: c.data }, blockNumber], responseHandler(c.id));
        });
        return this.rpc.batch(requests).then(function () {
          return results;
        });
